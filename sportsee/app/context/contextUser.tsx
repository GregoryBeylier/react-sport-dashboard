...
export const UserProvider = ({ children }: { children: ReactNode }) => {
  ...
  const totalDistance = userData?.runningData.reduce((acc, session) => {
    return acc + session.distance
}, 0) ?? 0

  return (
   <ContextUser.Provider value={{ firstName, lastName, photoProfile, isLoading, createdAt, totalDistance }}>
      {children}
